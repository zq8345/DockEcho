...
- Isolated-note hints
- Resurfacing view
- One-click connection discovery for the current note
- Folder mode (desktop Chrome/Edge): mount your existing Obsidian / Markdown folder in place — files stay untouched unless you edit them, deletion moves files to `.trash/`, app metadata lives in `.dockecho/meta.json`
- Today's Echo: at most one resurfaced note per day, always with a reason why it came back (open / insert link / later / not relevant)
- Note deletion
...
- `index.html` — marketing landing page (bilingual, founding-member waitlist via Netlify Forms)
- `app.html` — the app itself

## Usage

Open `app.html` directly — no server, no build step, no dependencies. Old notes can be brought in via the importers (Kindle `My Clippings.txt`, Readwise CSV, or a batch of Markdown files), parsed entirely on your device.
...
- 知识回流视图
- 一键发现当前笔记的连接
- 文件夹模式（桌面版 Chrome/Edge）：直接挂载你现有的 Obsidian / Markdown 文件夹——未编辑的文件不会被改动，删除只移入 `.trash/`，应用元数据存于 `.dockecho/meta.json`
- 今日回声：每天最多一张回流卡片，必须带"为什么回来"（打开 / 插入链接 / 稍后 / 不相关）
- 删除笔记
- 单篇 Markdown 导出
- 全库 Markdown 导出
- 明暗主题
...
- `index.html` —— 营销落地页（双语，创始会员候补表单走 Netlify Forms）
- `app.html` —— 应用本体

## 使用

直接打开 `app.html` 即可使用，不需要服务器和安装依赖。旧笔记可通过导入器带回（Kindle `My Clippings.txt`、Readwise CSV、批量 Markdown），全部在本地设备上解析。
